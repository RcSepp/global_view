...
namespace csharp_viewer
{
	...
	{
		public delegate void SelectionChangedDelegate(Selection selection);
		...
		private GLShader sdrColored;
		private int sdrColored_colorParam;
		private GLMesh meshSelection;

		private Selection selection;

		private Rectangle[] argumentLabelBounds;

		private class TrackBar //TODO
		{
			public string label = "";
		...
		public ArgumentIndex(Dictionary<int[], TransformedImage> images)
		{
			this.images = images;
		}
		...
		public void Draw(Point mousepos, Size backbufferSize)
		{
			...
			{
				trackbar.Draw(bounds, selection[i++], backbufferSize, sdrColored, sdrColored_colorParam); //TODO: make constraint sets indexable. Otherwise selection[i++] will crash for meta data trackbars
		...
		{
			for(int i = 0; i < argumentLabelBounds.Length; ++i)
				if(argumentLabelBounds[i].Contains(e.Location))
				{
...
else
{
		...
		/*public static IEnumerable<int[]> EnumerateSelectionGroup(HashSet<int>[] selectiongroup)
		{
			HashSet<int>.Enumerator[] selectioniter = new HashSet<int>.Enumerator[selectiongroup.Length];
			int[] selectionkey = new int[selectiongroup.Length];
			for(int i = 0; i < selectiongroup.Length; ++i)
			{
				selectioniter[i] = selectiongroup[i].GetEnumerator();
		...
		public static bool SelectionGroupContains(HashSet<int>[] selectiongroup, int[] key)
		{
			int i = 0;
			foreach(HashSet<int> indices in selectiongroup)
